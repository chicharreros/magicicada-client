...
class FakedCredentialsManagementTool(object):
    """A faked CredentialsManagementTool."""

    credentials = FAKED_CREDENTIALS
    should_fail = False
    ...
    def login(self, *args, **kwargs):
        """Fake login."""
        if self.should_fail:
            return defer.fail(self.should_fail)
        self._called['login'] = (args, kwargs)
        return defer.succeed(self.credentials)
...
class SyncdaemonServiceConnectTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.patch(interaction_interfaces, 'NetworkManagerState',
                   FakeNetworkManagerState)
        self.patch(credentials, 'CredentialsManagementTool',
    ...
    def test_connect_pushes_SYS_USER_CONNECT_with_the_token(self):
        ...
        yield self.sd_obj.connect(autoconnecting=self.autoconnecting)
        self.assertEqual(self.events, [('SYS_USER_CONNECT',
                                       {'access_token': FAKED_CREDENTIALS})])

    @defer.inlineCallbacks
    def test_connect_raises_exception_if_no_token(self):
        """If no credentials, NoAccessToken is raised."""
        self.patch(FakedCredentialsManagementTool, 'credentials', {})
        d = self.sd_obj.connect(autoconnecting=self.autoconnecting)
        yield self.assertFailure(d, NoAccessToken)

    @defer.inlineCallbacks
    def test_connect_raises_exception_if_auth_denied(self):
        """If no credentials, NoAccessToken if user denied auth."""
        self.patch(FakedCredentialsManagementTool, 'credentials', None)
        d = self.sd_obj.connect(autoconnecting=self.autoconnecting)
        yield self.assertFailure(d, NoAccessToken)

    @defer.inlineCallbacks
    def test_exceptions_are_handled(self):
        """Every Exception is handled."""
        expected = Exception('test me')
        self.patch(FakedCredentialsManagementTool, 'should_fail', expected)
        d = self.sd_obj.connect(autoconnecting=self.autoconnecting)
    ...
    @defer.inlineCallbacks
    def test_auth_credentials_are_used_to_connect(self):
...
class AutoconnectingTestCase(SyncdaemonServiceConnectTestCase):
    """Tests the interaction with the credentials.

    Check conditions when autoconnecting is True.
