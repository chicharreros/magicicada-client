...
from magicicadaclient.networkstate.networkstates import ONLINE, OFFLINE
...
class NetworkManager(DesignatedWrapPolicy):
    ...
    def __init__(
        self, connected_cb=None, connected_cb_info=None, disconnected_cb=None
    ):
        self._wrap_(self)
    ...
    def register(self):
        ...
        for current_event in SUBSCRIPTIONS:
            ...
            try:
                event_system.Store(
                    PROGID_EventSubscription, event_subscription
                )
    ...
    def find_online_state(self, listener=None, listener_thread=None):
        ...
        if listener_thread is None:
            listener_thread = Thread(
                target=listener.register, name="Network Connection Monitor"
            )
