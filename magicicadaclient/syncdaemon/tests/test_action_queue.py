...
class TestZipQueue(BasicTestCase):
    ...
    def test_compress_gets_compressed_data(self):
        ...
        upload = FakeUpload()
        data = b"a lot of data to compress"

        # call and wait
        d = defer.Deferred()
        reactor.callInThread(self.zq._compress, d, upload, BytesIO(data))
        yield d

        with open_file(upload.tempfile.name, 'rb') as f:
            compressed = f.read()
        self.assertEqual(compressed, codecs.encode(data, 'zip'))
...
class UploadUnconnectedTestCase(FactoryBaseTestCase):
    ...
    def test_client_request(self):
        ...
        data = b"content to be sent in the upload"
        self.command.tempfile = BytesIO(data)
...
class UploadProgressWrapperTestCase(BaseTwistedTestCase):
    ...
    def test_seek(self):
        ...
        f = BytesIO(b"v" * 10 + b"w" * 10 + b"x" * 5 + b"y" * 5)
        cmd = FakeCommand()
        upw = UploadProgressWrapper(f, cmd)
...
class UploadTestCase(ConnectedBaseTestCase):
    ...
    def test_uniqueness_tried_to_cancel_but_no(self):
        ...
        self.patch(Upload, 'run', lambda self: defer.Deferred())
        self.action_queue.upload('foo', 'bar', 0, 0, 0, 0, self.mdid, BytesIO)
