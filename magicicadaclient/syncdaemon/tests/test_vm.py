...
class BaseVolumeManagerTests(BaseTwistedTestCase):
    ...
    def setUp(self):
        ...
        self.root_dir = self.mktemp(os.path.join('chicharrero', 'root_dir'))
    ...
    def _create_udf(
        ...
        volume = self._create_udf_volume(
            volume_id=volume_id,
            node_id=node_id,
            suggested_path=suggested_path,
            generation=generation,
...
class VolumeManagerSharesTests(BaseVolumeManagerTests):
    ...
    def test_share_is_equal(self):
        ...
        share1 = self._create_share(
            volume_id='volume_id', node_id='node_id', subscribed=True
        )
        share2 = self._create_share(
            volume_id='volume_id', node_id='node_id', subscribed=True
        )
    ...
    def test_share_is_not_equal(self):
        ...
        share2 = self._create_share(
            volume_id='volume_id', node_id='node_id', subscribed=False
        )
    ...
    def test_add_share_access_level_view(self):
        ...
        share = self._create_share(
            access_level=ACCESS_LEVEL_RO, subscribed=False
        )
    ...
    def test_add_share_access_level_modify(self):
        ...
        share = self._create_share(
            access_level=ACCESS_LEVEL_RW, subscribed=False
        )
        yield self.vm.add_share(share)
    ...
    def test_add_share_view_does_not_local_scan_share(self):
        ...
        share = self._create_share(
            access_level=ACCESS_LEVEL_RO, subscribed=True
        )
    ...
    def test_share_changed(self):
        ...
        share = Share(
            path=share_path,
            volume_id=share_holder.share_id,
            access_level=ACCESS_LEVEL_RO,
        )
        yield self.vm.add_share(share)
        self.vm.share_changed(share_holder)
        self.assertEqual(
            ACCESS_LEVEL_RW, self.vm.shares[share.volume_id].access_level
        )
    ...
    def test_add_shared(self):
        ...
        def fake_create_share(node_id, user, name, access_level, marker, path):
            ...
            self.vm.handle_AQ_CREATE_SHARE_OK(
    ...
    def test_create_share_missing_node_id(self):
        ...
        def fake_create_share(node_id, user, name, access_level, marker, path):
            ...
            self.main.event_q.push(
                "AQ_CREATE_SHARE_OK", share_id=expected_share_id, marker=marker
            )
    ...
    def test_delete_shared_error(self):
        ...
        events = yield d
        self.assertEqual(
            events[0], dict(share_id=share.volume_id, error='a fake error')
        )
    ...
    def test_remove_watch(self):
        ...
        yield self.vm._add_watch(path)
        self.assertIn(
            path, self.watches, 'watch for %r should be present.' % path
        )
        self.vm._remove_watch(path)
        self.assertNotIn(
            path, self.watches, 'watch for %r should not be present.' % path
        )
    ...
    def test_handle_AQ_ANSWER_SHARE_OK(self):
        ...
        self.assertTrue(
            self.main.fs.get_by_path(share.path),
            'No metadata for share root node.',
        )
        self.assertTrue(path_exists(share.path), 'share path missing on disk!')
    ...
    def test_handle_AQ_DELETE_SHARE_ERROR(self):
        ...
        self._listen_for('VM_SHARE_DELETE_ERROR', d.callback, 1, collect=True)
        self.main.event_q.push(
            'AQ_DELETE_SHARE_ERROR', share_id='share_id', error='a fake error'
        )
        events = yield d
        self.assertEqual(
            events[0], dict(share_id=share.volume_id, error='a fake error')
...
class ViewSharesSubscriptionTests(BaseVolumeManagerTests):
    ...
    def test_subscribe_share(self):
        ...
        share = self._create_share(
            access_level=self.access_level, subscribed=False
        )
        yield self.vm.add_share(share)
    ...
    def test_unsubscribe_share(self):
        ...
        share = self._create_share(
            access_level=self.access_level, subscribed=True
        )
        yield self.vm.add_share(share)
        self.assertTrue(self.vm.shares[share.volume_id].subscribed)
        # unsubscribe from it
...
class VolumeManagerStringManagementTests(BaseVolumeManagerTests):
    ...
    def test_handle_SHARES_sharename(self):
        ...
        # check
        share = self.vm.shares['share_id']
        shouldbe_dir = os.path.join(
            self.shares_dir, get_share_dir_name(share_response)
        )
    ...
    def test_handle_SV_SHARE_CHANGED_sharename(self):
        ...
        self.vm.handle_SV_SHARE_CHANGED(info=share_holder)
        shouldbe_dir = os.path.join(
            self.shares_dir, get_share_dir_name(share_holder)
        )
...
class VolumeManagerVolumesTests(BaseVolumeManagerTests):
    ...
    def test_udf_ancestors(self):
        ...
        expected = [
            ...
            os.path.join('~', 'Documents', 'Reading Años', 'Books'),
        ]
    ...
    def test_get_volume(self):
        ...
        self.assertEqual(1, len(self.vm.udfs))
        self.assertEqual(2, len(self.vm.shares))
        self.assertEqual(udf.volume_id, self.vm.get_volume(str(udf_id)).id)
...
class HandleListVolumesTestCase(BaseVolumeManagerTests):
    ...
    def test_handle_AQ_LIST_VOLUMES(self):
        ...
        share_volume = self._create_share_volume(
            volume_id=share_id, name=share_name, node_id=share_node_id
        )
        udf_id = uuid.uuid4()
        udf_node_id = 'yadda-yadda'
        suggested_path = '~/UDF'
        udf_path = get_udf_path(suggested_path)
        udf_volume = self._create_udf_volume(
            volume_id=udf_id,
            node_id=udf_node_id,
            suggested_path=suggested_path,
        )
        ...
        d1 = defer.Deferred()
        d2 = defer.Deferred()
        self.vm.refresh_volumes = lambda: d1.errback(
            Exception('refresh_volumes called!')
        )
    ...
    def test_handle_AQ_LIST_VOLUMES_non_ascii(self):
        ...
        name = 'ñoño'
        share_volume = self._create_share_volume(
            volume_id=share_id, name=name, node_id='fake_share_uuid'
        )
        udf_id = uuid.uuid4()
        udf_volume = self._create_udf_volume(
            volume_id=udf_id,
    ...
    def test_handle_AQ_LIST_VOLUMES_root(self):
        ...
        self.assertIn(request.ROOT, self.vm.shares)
        self.assertEqual(
            self.vm.shares[request.ROOT].node_id, str(root_volume.node_id)
        )
...
class VolumeManagerOpTestsRequiringRealFSMonitor(BaseVolumeManagerTests):
    ...
    def test_add_udf_with_content(self):
        ...
        suggested_path = "~/suggested_path"
        udf = self._create_udf(suggested_path=suggested_path, subscribed=True)
...
class VolumeManagerOperationsTests(BaseVolumeManagerTests):
    ...
    def test_handle_AQ_CREATE_UDF_ERROR(self):
        ...
        def create_udf(path, name, marker):
            """Fake create_udf"""
            self.main.event_q.push(
                "AQ_CREATE_UDF_ERROR", marker=marker, error="ERROR!"
            )
    ...
    def test_handle_AQ_DELETE_VOLUME_ERROR(self):
        ...
        def delete_volume(vol_id, path):
            """Fake delete_volume"""
            self.main.event_q.push(
                "AQ_DELETE_VOLUME_ERROR", volume_id=vol_id, error="ERROR!"
            )
    ...
    def _test_handle_SV_VOLUME_CREATED_share(self, auto_subscribe):
        ...
        # start the test
        share_volume = self._create_share_volume(
            accepted=False, access_level=ACCESS_LEVEL_RW
        )
        # initialize the the root
        self.vm._got_root('root_uuid')
    ...
    def test_handle_SV_VOLUME_DELETED(self):
        ...
        udf_volume = volumes.UDFVolume(
            udf_id, 'udf_uuid', None, 10, suggested_path
        )
    ...
    def test_get_volumes_share_inactive(self):
        """Tests for VolumeManager.get_volumes."""
        share_no_accepted = self._create_share(accepted=False, subscribed=True)
        yield self.vm.add_share(share_no_accepted)
        share_no_subscribed = self._create_share(
    ...
    def test_udf_from_udf_volume(self):
        ...
        path = get_udf_path(suggested_path)
        volume = volumes.UDFVolume(
            uuid.uuid4(), uuid.uuid4(), None, 10, suggested_path
        )
    ...
    def test_no_UDFs_outside_home(self):
        ...
        def create_udf(path, name, marker):
            """Fake create_udf"""
            d = dict(
                volume_id=uuid.uuid4(), node_id=uuid.uuid4(), marker=marker
            )
    ...
    def test_no_UDFs_inside_root(self):
        ...
        self.assertEqual(
            result, dict(path=udf_path, error="UDFs can not be nested")
        )
    ...
    def test_no_UDFs_inside_udf(self):
        ...
        self.assertEqual(1, len(list(self.vm.udfs.keys())))
        self.assertEqual(
            result, dict(path=udf_child, error="UDFs can not be nested")
        )
    ...
    def test_UDF_ok_name_similar_shorter(self):
        ...
        self._listen_for('VM_UDF_CREATED', d.callback)
        self._listen_for(
            'VM_UDF_CREATE_ERROR', lambda r: d.errback(Exception(r))
        )
    ...
    def test_dont_delete_volumes_on_handle_AQ_SHARES_LIST(self):
        ...
        response = [share_volume, udf_volume, root_volume]
        d = defer.Deferred()
        self.vm.refresh_volumes = lambda: d.errback(
            Exception('refresh_volumes called!')
        )
        self._listen_for('VM_UDF_CREATED', d.callback)
        self.vm.handle_AQ_LIST_VOLUMES(response)
    ...
    def test_UDF_cant_be_a_symlink(self):
        ...
        self.assertEqual(0, len(list(self.vm.udfs.keys())))
        self.assertEqual(
            result, dict(path=udf_path, error="UDFs can not be a symlink")
        )
    ...
    def test_server_rescan(self):
        ...
        self._listen_for(
            'SV_VOLUME_NEW_GENERATION', vol_rescan_d.callback
    ...
    def test_server_rescan_with_share_autosubscribe(self):
        ...
        share_id = uuid.uuid4()
        share_volume = self._create_share_volume(
            volume_id=share_id, generation=17
        )
        udf_id = uuid.uuid4()
        udf_volume = volumes.UDFVolume(udf_id, 'udf_node_id', 13, 200, '~/UDF')
        root_id = uuid.uuid4()
        root_volume = volumes.RootVolume(root_id, 1, 500)
        response = [share_volume, udf_volume, root_volume]
        ...
        vol_rescan_d = defer.Deferred()
        self._listen_for(
            'SV_VOLUME_NEW_GENERATION', vol_rescan_d.callback, 1, collect=True
        )
        server_rescan_d = defer.Deferred()
        self._listen_for('SYS_SERVER_RESCAN_DONE', server_rescan_d.callback)
        yield self.vm.server_rescan()
    ...
    def test_volumes_rescan_cb(self):
        ...
        share_id = uuid.uuid4()
        share_volume = self._create_share_volume(
            volume_id=share_id, generation=1
        )
        udf_id = uuid.uuid4()
        udf_volume = volumes.UDFVolume(udf_id, 'udf_node_id', 1, 200, '~/UDF')
        root_id = uuid.uuid4()
        ...
        yield udf_d
        events_dict = dict(
            (event['volume_id'], event['generation']) for event in events
        )
    ...
    def test_volumes_rescan_cb_handle_root_node_id(self):
        ...
        self.assertTrue(
            self.main.fs.get_by_node_id(request.ROOT, str(root_id))
        )
    ...
    def test_volumes_rescan_cb_inactive_volume(self):
        ...
        udf_id = uuid.uuid4()
        udf_volume = volumes.UDFVolume(
            udf_id, uuid.uuid4(), 10, 100, suggested_path
        )
        udf = UDF.from_udf_volume(udf_volume, path)
        root_id = uuid.uuid4()
        root_volume = volumes.RootVolume(root_id, 1, 500)
    ...
    def test_volumes_rescan_cb_missing_fsm_md(self):
        ...
        udf_id = uuid.uuid4()
        udf_volume = volumes.UDFVolume(
            udf_id, 'udf_uuid', 10, 100, suggested_path
        )
        udf = UDF.from_udf_volume(udf_volume, path)
        udf.subscribed = True
        ...
        events_dict = dict(
            (evt['volume_id'], evt['generation']) for evt in events
        )
    ...
    def test_update_generation(self):
        ...
        self.assertRaises(
            VolumeDoesNotExist, self.vm.update_generation, str(uuid.uuid4()), 1
        )
    ...
    def test_handle_SV_VOLUME_NEW_GENERATION_udf(self):
        ...
        self.main.event_q.push(
            'SV_VOLUME_NEW_GENERATION', volume_id=udf.volume_id, generation=100
        )
        vol_id, gen = yield d
    ...
    def test_handle_SV_VOLUME_NEW_GENERATION_root_eq(self):
        ...
        self.assertEqual(
            msg % (root.volume_id, 100, 100), self.handler.records[0].message
        )
...
class MetadataTestCase(BaseTwistedTestCase):
    ...
    def setUp(self):
        ...
        self.root_dir = self.mktemp(os.path.join('chicharrero', 'Magicicada'))
...
class MetadataUpgraderTests(MetadataTestCase):
    ...
    def test_guess_metadata_version_5(self):
        ...
        shelf['foobar'] = _Share(
            path=os.path.join('foo', 'bar'), share_id='foobar'
        )
        version = self.md_upgrader._guess_metadata_version()
    ...
    def test_guess_metadata_version_6(self):
        ...
        shelf['foobar'] = Share(
            path=os.path.join('foo', 'bar'), volume_id='foobar'
        )
        version = self.md_upgrader._guess_metadata_version()
    ...
    def test_guess_mixed_metadata_5_and_6(self):
        ...
        # fake a version 6 layout and metadata
        shelf = LegacyShareFileShelf(self.share_md_dir)
        shelf['old_share'] = _Share(
            path=os.path.join('foo', 'bar'), share_id='old_share'
        )
        shelf['new_share'] = Share(
            path=os.path.join('bar', 'foo'), volume_id='new_share'
        ).__dict__
        version = self.md_upgrader._guess_metadata_version()
        self.assertEqual(version, '5')
...
class VMTritcaskShelfTests(BaseTwistedTestCase):
    ...
    def test_get_key(self):
        ...
        self.assertEqual(
            self.shelf._root_key, self.shelf._get_key(request.ROOT)
        )
